...
public:
	...
	CyclicTime(
        time_t & time
    ,   time_t & utcOffset ///< In seconds (UTC+01:00 is m_utcOffset = +3600).
    ,   t_longf secondsPerCycle = 0.0);
    ...
    */
    ...
    t_longf getSecondsPerCycle() const;

    t_longf setSecondsPerCycle(const t_longf secondsPerCycle);


	/** Float time in the intervall [0;1]
    */
    inline t_longf getf() const { return m_timef[1]; }
...
protected:
    static t_longf secondsTof(const time_t & time);
    static time_t fToSeconds(t_longf time);
